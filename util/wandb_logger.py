...
class WandbLogger:
    ...
    def __init__(self, opt):
        ...
        if self.config.get('log_eval', None):
            self.eval_table = self._wandb.Table(columns=['origin_image',
                                                         'noise_image',
                                                         'enhanced_image',
                                                         'psnr',
                                                         'ssim',
        ...
        if self.config.get('log_infer', None):
            self.infer_table = self._wandb.Table(columns=['origin_image',
                                                         'noise_image',
                                                         'enhanced_image',
                                                          'UIQM',
                                                          'UCIQE'])
    ...
    def log_eval_data(self, origin_image, noise_image, enhanced_image, psnr=None, ssim=None,LPIPS=None,UIQM=None,UCIQE=None):
        """
        Add data row-wise to the initialized table.
        """
        if psnr is not None and ssim is not None and LPIPS is not None:
            self.eval_table.add_data(
                self._wandb.Image(origin_image),
                self._wandb.Image(noise_image),
                self._wandb.Image(enhanced_image),
                psnr,
                ssim,
                LPIPS
            )
        else:
            assert UIQM is not None and UCIQE is not None
